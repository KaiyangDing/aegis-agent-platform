...
| `experiment_kill9_ingest_linux.py` | **#48 Linux 复验**（#26 追加验收义务）：容器 prefork/asynloop 世界里 kill -9 → **零人工**等 kombu 定时器自动 restore → 重新消费至 DONE（Windows 实录缺的那一环）；种子/断言/清理件 importlib 复用 `experiment_kill9_ingest.py`（该件 M4.7 起带 main 守卫可复用）；自动停/复 compose worker+beat、自起假服务与 kill9 容器（scripts/ 只读卷挂载）。凭证 `reports/m4_kill9_ingest_linux.txt` | M4.7-E | 否（假 embedding 顶替上游） | aegis-app 镜像已构建 + PG/Redis 容器 |
