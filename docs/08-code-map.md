...
| 既有 tag | `m0-foundation`、`m1-gateway`、`m2-runtime` | `git tag` |
...
注意：项目根**没有 README.md、没有 Dockerfile**（应用容器化归 M4.7，00 §10.1 #26）。
...
- 仓库根新增：`data/corpus/{tenant-a,tenant-b}/`（各 10 篇语料）、`evals/`（README+cases/seed.jsonl 20 条；M4.4 +cases.json、M4.6 +cost_questions.json）、`tests/cassettes/l3/`（五盘）；迁移 2→**8**（M4.0④b 更正，原记 9）；scripts 13→**30**（M3.12 时 24，M4.0④b +3，M4.6 +3）（§9.1）；测试文件 42→60+（增量见 §0-bis 各节）。
