...
> <1min，"实测 ≤15 分钟"判定成立）。
>
> **M5.5 落地实况（2026-08-04）**：README 终稿（mermaid 架构图/高光能力表/数字表带口径与
> 凭证链接/demo 入口/AI 协作声明——初稿 8 节结构全兑现；Quickstart 改容器全栈主形态）；
> 05 回填=§10.2 七数字全部入句、#34 调序（runtime/replay 置顶、网关降佐证语气）、#27 精简版
> 校验过（前三行 replay 在）、#28 DeepSeek 字样移除（06 已在 M5.0 同步）、90 秒叙事第 3 拍
> 默认 replay；**§4 AI 协作三段随 M4.6+ 全量委托例外更新**（逐词诚实：git Co-Authored-By
> 尾注=分工可查证据）；atlas 三骨架终稿（失败哲学 21 条并表/数字卡 M4 表头修复+M5 档/复习
> 路径三条/边界汇总 v1.0 版+M5 条目）；**P4 优雅停机顺手验 ✅**（计划 §4-M5.3-4 原文兑现，
> 在途流 SIGTERM 带 done 收尾、退出码全 0，凭证 `m5_graceful_shutdown.txt`，00 #31 行登记）。
>
> **M5.6 终验收实况（2026-08-04，tag `v1.0`）**：§9.2 四项全勾（对账表 00 §9.3 建齐，
> B=989→1003）；简历逐词对照走完（Anthropic 桩=providers/anthropic.py 在、Qwen 三档在
> routes、被砍项 coupons/语义缓存/DeepSeek 零出现）；深挖题 476–482 入库（§9-3 七题全领）；
> #27/#28/#34 翻 ✅、#33 **关闭（未做，如实）**——技能栏 LangGraph 行未写，P3 前提不满足；
> **M5 真实调用总账 ≤¥0.05**（ledger 圈定 ¥0.0154+口径② 盲区上界；lt-tenant ¥1.2757/1602 行
> =模拟流量记账痕迹非真实花费，声明入 00 §9.3 防未来误读）；本块即头部「实际落地偏差」
> 回填终稿（M5.0–M5.6 六条实况全在上）。
>
