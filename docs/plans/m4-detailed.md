...
>
> **M4.7 开工核对/拍板与交付实录（2026-08-03 六交付一天收口，测试 958→989，
> 七笔 `4a06503`A/`72d4eb0`B/`ac872cc`C/`a20033e`+`0f86b23`D/`7a0392d`E/`9aff73e`F前置+收口文档笔；
> 全程 AI 执行=M4.6 委托延续；与本章第八章不符处以本块为准）**：
> 交付切分实况：计划两件（迁 docs/容器化）+观察池 M4.7 档（§10.1-bis ~26 条）实切**六份**
> A（(67)(68) 前置件）→B（底座确定性 8 条）→C（服务层组 10 条）→D（迁入+修订两笔）→
> E（容器化+三实测）→F（㉝(64) 补件+回填）。
> (47) **原 kill9 脚本尾部 `asyncio.run(main())` 无 main 守卫**——Linux 复验脚本 importlib
> 装载即执行、在运行中 loop 里炸 RuntimeError（实测撞出）；补守卫后成可复用件
> （seed_demo 有守卫、它没有="脚本可复用性"此前无纪律，教训：**脚本一律带 main 守卫**）。
> (48) **compose scale 的第一道拦截是 container_name 不是端口**（§7-6 只预言了端口）——
> `--scale api=3` 原话 "Remove the custom name to scale the service"；M5.3 须同时去掉
> api 的 container_name 与宿主端口映射（compose 注释已预埋后半，前半本块登记）。
> (49) **celery eager 结果层重建异常按 str 单参重构**——ProviderError 家族双参签名重构
> 失败回退基类 GatewayError（探针实测）；⑬ 测试因此以计数器为契约不断言异常类身份
> （生产无 result backend 故零影响，纯测试面认知）。
> (50) **㉝ 分档偏差**：归位原文"不一致抛人话异常"，实装 mock=strict 抛、redis/http=
> 响亮警告（既有装置面广，一刀切会把"曾经侥幸可用"的路径切死）——989 全绿零误伤
> 实证分档正确；观察一个里程碑再议升级。(64) 实装为声明形态（对应清单+两条有意差异
> 落 docstring），机制钉子不做——危害是文档级，M5.4 demo 排练面复核。
> (51) **附A #9 后半按实况裁定**：容器冒烟真实调用"1 次"→**2 请求 4 调用 <¥0.01**
> （(57) 断连探针必须真调用——它是 M4.2 收口拍板 5 移位到本步的义务）；委托范围内
> 放行并三处登记（00 §8.3/凭证文件/本块）。
> (52) **Dockerfile 基底按计划 §3-2 字面落地**（`ghcr.io/astral-sh/uv:python3.13-bookworm-slim`
> 一次拉通）；api healthcheck 用 python urllib（slim 无 curl，计划预言成立）。
> (53) **(57) 实测半边范围声明**：chat SSE 真实断连全弧已实证；approvals 端点断连
> 全弧（带 HITL 剧本）并入 M5.4 demo 排练——本步只验"断连不取消生产者"的网络语义本体。
> **实测三凭证**：`m4_container_smoke.txt`（五项+(57)+scale）/`m4_kill9_ingest_linux.txt`
> （四断言：kill -9 后重启 **102s 零人工自动恢复至 DONE**、账本=理论批数零重复计费——
> Windows 实录缺的那一环闭合，#48 两个世界自此都有凭证）/仓外 `backups\aegis_dump_20260803.sql`
> （C41 首次 dump，2.97MB 不入库）。
>
> **M4.8 实况（2026-08-03 毕业收口）**：按第九章 §4 清单执行，两条偏差：
> (54) §8.2 第一条复选框在 M4.1 达成时漏翻（M4.6 收口时发现登记）——本步对账修正并
> 现场重演（容器 api 回放冒烟会话）；(55) "报告落盘"实体=`reports/m4_acceptance.md`
> （M3 先例 m3_acceptance 的 M4 版：四条凭证对账+真实调用总账 **¥1.2488** 账本五族
> 聚合+步账）。retro-m3.md 与 atlas 治理篇同批落笔（#50⑶/#40）；#33 弹性窗已向用户
> 点名。其余无偏差。
>
