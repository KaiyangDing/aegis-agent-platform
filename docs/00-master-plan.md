...
## 8. M4 · 治理层（1.5 周基线，9 步）✅ 2026-08-03 毕业（tag `m4-governance`，989 测试）
...
- [x] 凭 trace_id 还原任一会话每步输入输出与耗时——M4.1 凭证 `m4_trace_sample.json`+M4.8 现场重演（容器 api 回放冒烟会话；复选框此前漏翻=M4.6 收口时登记的漂移，本次对账修正）；
...
| M4.7-A ✅ | **(67)(68) 前置件（2026-08-03，测试 965→968）**：notifier 守连循环 +SELECT 1 心跳（独立 heartbeat 参数——不给 (70) 双职旋钮加第三职；黑洞形态发现时延从"永不/25s"压到心跳周期量级）+降级/停机 finally `_wake_all()`（降级开关对**在途**等待者生效，不再抱死 Event 熬满 timeout）。测试=确定性假连接注入（黑洞不可本地真实复现→probe_error 替身），3 新测 | `4a06503` |
| M4.7-B ✅ | **观察池修缮批一：底座确定性组（测试 968→976）**：①②目录缓存键含环境身份维度+返回深拷副本（缓存="RLS 结论的备忘录"而非旁路；既有 `second is first` 断言随行为改判）；③新迁移 `e5a1c7d94f02` REVOKE alembic_version（+test_rls M4.7 证人：低权 SELECT 也 42501）；⑦`auth.ttl_for` 单点+方向测试（写反=终端用户暴露窗放大 4 倍从此有红灯）；⑬摄取壳确定性错误（AuthError/BadRequestError）不重试直接 FAILED（celery eager 结果层重建双参异常回退基类=探针实测，行为断言用计数器）；⑳CJK 区间单点 `core.tokens` 三处同尺；㉔intent 解析挪进保护区（docstring 承诺变结构保证）；㊶spec config 快照 `is not` 证人 | `72d4eb0` |
| M4.7-C ✅ | **观察池修缮批二：服务层组（测试 976→984）**：㊸㊹直答判据+LLM 流式+写盘整体入会话锁段（回落在锁外——runtime 自取同锁，锁内回落=自撞 409，结构性防死锁断言钉死）；㊻兜底建单失败发**零承诺话术** `FALLBACK_LOOP_LIMIT_NO_TICKET`（500 空响应不可达；旧话术自称已建单把"失败照发"堵死）；㊵装配错误前置到 classify 之**前**拦截（error 帧+零花费证据 gw.calls==0）；(66)tools 轮流中守卫命中必留审计（不补 SAFE_REPLY——本轮无回复位，理由入码）；㉙(54)mock 回放读带租户过滤（无 RLS 世界也响亮 NoResultFound）；㊳/tickets 走 `_claim_and_execute`（同键同 ticket_id=崩溃重试零孤儿单，"写工具一律带键"3/3）；㊽create_handoff 走 post_write 单点带键；(60)a+ 认领前续租一次（打空=LeaseLost 自毁）；⑤(76)注入组合空间声明+装配日志；㊱(55)读侧合流前提声明（mock 只产 200/404/503，真实化前复核） | `ac872cc` |
| M4.7-D ✅ | **文档迁入+修订（内容见 §10.1 #9 行）** | `a20033e` `0f86b23` |
| M4.7-E ✅ | **应用容器化（内容见 §10.1 #26/#31/#48 行；凭证 `m4_container_smoke.txt`+`m4_kill9_ingest_linux.txt`）**。真实调用记账：容器冒烟 2 请求 4 调用 <¥0.01（附A #9 后半按实况裁定："1 次"修订为 2——(57) 断连探针必须真调用，委托范围内放行并登记） | `7a0392d` |
| M4.8 ✅ | **毕业验收+整编（2026-08-03，测试 989 不变，tag `m4-governance`）**：§8.2 四条全 ✅（第一条现场重演=容器 api 回放冒烟会话 TraceView；复选框漂移修正）；`reports/m4_acceptance.md`（四条凭证对账+**M4 真实调用总账 ¥1.2488**=账本五族聚合可复算+步账 854→989）；**`docs/retro-m3.md` 落笔**（#50⑶：十一站口径精华+缺陷终局对账+哲学九条+连环炮十二问）；**atlas 治理篇**（#40：图组 E 两幅+闸门治理档 8 行+文件表/数字卡/边界 M4 节）；§10.1 #9/#26/#31/#48 及 #50 全项终翻；#33 LangGraph spike 弹性窗已向用户点名（M4 毕业后，做不做用户排期）。毕业四件：CI 全绿+tag 推送+记忆重写+00 本表 | 毕业收口笔+tag |
| M4.7-F ✅ | **㉝(64) 补件+收口回填（测试 984→989）**：盘点抓漏两条——㉝ `core/loopcheck.LoopBoundGuard` 共性防线（首用绑定 loop/同实例跨 loop 严格档抛人话【mock】、非严格档响亮警告【redis/http，偏差登记先观察】/替身重绑定豁免；5 新测全绿且**零误伤既有 984**=分档决策实证）；(64) `_task_runtime` 装配对应清单+两条有意差异落 docstring（msgbuf 缺席=worker 续跑无 message_reset 的不对称首次登记在场）。§10.1-bis M4.7 档结清（哈希映射见其收束块）；kill9 原脚本补 main 守卫（无守卫 `asyncio.run` 被 importlib 装载即执行——实测撞出，成可复用件） | `9aff73e`+收口文档笔 |
...
| M5.4 | **15 分钟 demo 脚本**：三个高光时刻——故障注入（熔断+fallback）、断点续跑（HITL/kill -9）、多租户隔离（对抗演示）；实测计时 ≤15 分钟。**顺手补录两个凭证**：熔断恢复闭合时间实测（04 M1 验收未尽项，§10.1 #10）、**档内容灾切换实录（standard：qwen-plus→qwen-turbo——M5.0 P6 拍板修正：deepseek/glm 两版对象均已随 #28 作废，容灾=qwen 档内降级，M1 已有一次实测凭证与之连续；05 表述同步）** | M | 01 §3 |
...
| 40 | **项目全景复习图册 `docs/atlas.md`**（索引+图+速查，面试快速复习用；不重复 retro 深挖内容，深度一律链接跳转）：①全景/L1/L2/L3/治理五组 mermaid 图（三层架构、import 依赖、一次请求旅程、一次 run 双轨旅程、六道闸门位置、append 三岔口、六层预算、回放四道、隔离四层防线等）；②**闸门与护栏总表**（全项目拦截点逐行：名称/层/防什么/触发条件/失败方向 open·closed/代码锚点/测试锚点）；③每层文件表（文件·行数/一句话职责/最重要不变量/深挖跳转）；④失败哲学总表（M1 七条+M2 八条合并对照）；⑤数字凭证卡（简历数字→口径→reports/ 凭证）；⑥已知边界汇总（各 retro §7 聚合）；⑦复习路径（15 分钟速览+面试前一天清单）。**增量维护**：各里程碑毕业时补对应节（§13 第 5 项已挂），M5.5 终稿与简历回填同步 | 用户 2026-07-16 点名（M2 逐文件复盘会话，面试快速复习需求） | **M2.13 建骨架** → **M3.12 L3 篇 ✅** → **M4.8 治理篇已补 ✅**（2026-08-03：图组 E 两幅（治理五件套/容器拓扑）+闸门治理档 8 行+文件表治理节+数字卡 M4 六行+边界 M4 六条）→ **M5.5 终稿** | ⬜（M5.5 终稿余量） |
...
| # | 一句话 | 档 | 归位 / 理由 |
|---|---|---|---|
| ① | `TenantDirectory._users` 缓存键不含租户维度 | **升** | **✅ M4.7 已修**：当前不可达（请求路径不查 users），一旦为"即时降权"接上 `get_user` 立刻变可达越权面 |
| ② | 缓存存 detached 且**共享可变**的 ORM 实例 | **升** | **✅ M4.7 已修**：`config` dict 被就地改即污染全进程 60s；修=返回副本 |
| ③ | `GRANT ON ALL TABLES` 把 `alembic_version` 也授给 `aegis_app` 且该表无 RLS | **升** | **✅ M4.7 已修**：一行 REVOKE 闭合；"粗粒度授权换维护便利"的代价 |
| ④ | 02 §7.1"终端用户=租户侧签发的短期 JWT"与 HS256 **架构上不相容** | **升** | **✅ M4.7-D 已修（纯文档）**：对称密钥真给租户=隔离归零；改叙事或注明"v2 且届时必须换非对称" |
...
| ⑦ | `jwt_user_ttl_s`/`jwt_staff_ttl_s` 唯一消费点是 `mint_token.py:24` 三元表达式且**零测试** | **升** | **✅ M4.7 已修**：写反=终端用户暴露窗放大 4 倍且无红灯；修=抽 `auth.ttl_for(role)` 顺带覆盖 |
...
| ⑬ | Celery 壳 `except Exception` 无差别重试**抵消** EmbeddingClient 的白名单 | **升** | **✅ M4.7 已修**：AuthError 被重试 6 轮 18 次注定失败；修=壳加 `except (AuthError, BadRequestError)` 直接 mark_failed |
...
| ⑳ | CJK 范围 `一-鿿` 全仓三处内联无命名常量 | **升** | **✅ M4.7 已修**：tokens.py:15 / ingest.py:122 / rerank.py:24，"同一把尺"仅靠注释维系；修=core 导出常量一行 |
...
| ㉔ | `classify` docstring 承诺"解析 bug 也不该杀请求"，但 `_parse_intent` 在 try 外 | **升** | **✅ M4.7 已修**：今天不出事全靠它恰好是全函数=**未声明未测试的隐式前提**；对照组=guardrails 把解析放在被保护区内故是结构保证 |
...
| ㉙ | `_claim_and_execute` 回放分支的 SELECT **不带租户过滤**（与 claim 的 INSERT 不对称） | **升** | **✅ M4.7 已修**（与 (54) 合并）：今天安全全靠"键是 uuid4 不可控"+"RLS 在场"两条外部条件；幂等键一旦改成业务可控值立刻变跨租户读台账 |
...
| ㉝ | 跨 event loop 单例陷阱**三次现形、三种修法、零共性防线** | **升** | **✅ M4.7 已修**：判据=消费方有没有"你能塞参数进去"的时刻；最小机制形态=单例记录创建时 loop、不一致抛人话异常而非等 keep-alive 炸裸 RuntimeError |
...
| ㊱ | 读侧 `raise_for_status()` 把协议错（400 类）与可重试故障（503）合流成同一种 ERROR | **升** | **✅ M4.7 已修**：与写侧 #43 的精细区分粒度相反；当前 mock 读端点只产 200/404/503 故不可达，真实化前必须复核 |
| ㊲ | 退款闸门缺省 fail-OPEN | — | **✅ 已修**（M4.0②，`29a3b8b`） |
| ㊳ | `ticket_create` 送出的幂等键被下游**静默丢弃**（`POST /tickets` 连 header 都不读） | **升** | **✅ M4.7 已修**（与 ㊽ 合并）："写工具一律带键"兑现率 2/3；修=`/tickets` 也走 `_claim_and_execute` |
...
| ㊵ | docstring"配置错误启动炸"不符实：未知工具名是**每请求装配期** ValueError，无人接 → **500 空流** | **升** | **✅ M4.7 已修**：且发生在 `classify` 花完钱之后；真启动炸是 `@tool` import 期防呆 |
| ㊶ | `dict(tenant.config)` 只拷一层 + 无 CI 证人 | **升** | **✅ M4.7 已修**：钉子=一行 `is not` 断言 |
| ㊷ | 直答"先答后写"崩溃窗未登记（用户看到完整答案、事件流零记录、msgbuf 却有残影） | 边界 | 与"先答后写"的取舍成对（失败零残留 vs 崩溃窗） |
| ㊸ | 直答 LLM 流式**全程无会话锁**（锁只包 `_write_direct`）→ 并发窗内可被主 run 抢锁 | **升** | **✅ M4.7 已修**（与 ㊹ 合并）：用户看完答案却收 error 帧且答案不入事件流 |
...
| ㊻ | **兜底路径②不 fail-safe**（预算类→500 空响应比不兜底信息量更少／循环类→error 帧／建单成功后崩→孤儿工单） | **升** | **✅ M4.7 已修**：`FALLBACK_LOOP_LIMIT` 自称"已生成工单"堵死"失败照发"修法；修向 (a) 加不承诺工单的话术+try ／ (c) `create_handoff` 走 `post_write`，两者正交 |
...
| ㊽ | `create_handoff` 绕过 `post_write` 单点（无幂等键无 #43 分界） | **升** | **✅ M4.7 已修**（与 ㊳ 合并）：病根=契约被绑在"谁调用"而不是"这是一次外部写"上 |
...
| (57) | 审批端点响应时长=整个 run，无流式无上界 | **升** | **✅ 措辞半已修（M4.4③，`69959ae`）**：零事件态 `resumed`→`no_op`（词即语义，零消费方断言）；**✅ 实测半边已闭合（M4.7-E）**：容器 api 真实网络断连探针——首帧后硬断连，五事件完整落盘（凭证 m4_container_smoke.txt；approvals 全弧带 HITL 剧本的断连专项并入 M5.4 demo 排练，如实声明）；流式化改造不做（大改与评测无关，v2） |
...
| (60) | `lease.acquire`→`_pump_with_lease` 之间**无续租**，a+ 把这段拉长到含 precheck+工具执行+两次全事件扫描 | **升** | **✅ M4.7 已修**：`lease_ttl_s=60` 边际约 2× 且上界从未论证 |
...
| (64) | `_task_runtime` 与 `create_app` 的"四件逐件对应"只是**人肉承诺**（机制上区分不了"有意不加"与"忘了加"） | **升** | **✅ M4.7 已修**：证据是 `msg_redis`（worker 无 `_TokenEmitter` 故不写 msgbuf → worker 驱动续跑期间重连收不到 `message_reset`）；**站 10 原举例 `text_sink` 已更正**（它是每请求物不是装配参数） |
| (65) | `text_sink` 穿透四路径，生产只有 `service.py:306` 一条带非 None sink（三处 resume 调用点全不传） | 边界 | resume 侧穿透生产空转、唯一证人是测试；与 (64) 同源 |
| (66) | **有通道模式下 tools 轮守卫命中零审计**（`_finish_text` 只在 text 分支调用，`guard.hit` 无人检查） | **升** | **✅ M4.7 已修**：不发 GUARDRAIL_TRIGGERED 不发 SAFE_REPLY、下轮换新实例即遗忘——"守卫命中必留审计"在 tools 轮不成立 |
| (67) | **断连不唤醒在途等待者**（`_run` finally 只置 `_conn=None` 不遍历 `_waiters`） | **升** | **✅ M4.7-A 已修**（`4a06503`：降级/停机 finally 唤醒 _waiters）：改 notifier 守连循环风险中等、与 /metrics 无关——容器化前单独一笔做 |
| (68) | **无心跳、黑洞形态静默失聪** → 失效模式 25s 比设计好的降级模式 2s **慢 12.5× 且无日志** | **升** | **✅ M4.7-A 已修**（`4a06503`：SELECT 1 心跳独立参数——不给 (70) 那个旋钮加第三职；发现时延压到心跳周期量级） |
...
**M4.7 档结清**：升级项全数处置（修 24／声明收口 4（㊱(55)(64)(76 与⑤合并)）／⑫㉑㊺(72)(77) 各按既定移位不动）。
...
| 占位符 | 产出于 | 凭证 | 状态 |
|---|---|---|---|
| 故障注入成功率 X% + P99 代价 X s | M1.13 | `reports/m1_fault_injection.txt`（100% / 2.42s，口径见 §5.2）。**已回填 05 §1**（30% 上游失败 ×1000，P99 延迟代价随数字走） | ✅ 已回填 |
| 档位路由降本 X% | M4.6 实验① | **✅ 已产出**（`reports/m4_cost_routing.txt`，2026-08-03）：**vs-strong 74.7%／vs-standard 18.9%**——双基线口径（防"基线选贵抬数字"）；80 条全唯一集（与评测集零交集 CI 钉死）、分布 30/40/20/10 声明性假设、缓存关闭、演示价目、B 组含 fast 分诊成本；数字可由 usage_ledger 复算（报告附 SQL+sid 清单）。**已回填 05 §1**（双基线口径入句） | ✅ 已回填 |
| 精确缓存降本 X% | M4.6 实验② | **✅ 已产出**（`reports/m4_cost_cache.txt`，2026-08-03）：**21.9%**（30% 历史复述假设显式声明、seed=42 流量可精确重放；请求级全命中自检 60/60 与设计吻合；精确缓存上界口径=逐字节命中，语义缓存归 v2）。**已回填 05 §1**（30% 复述假设入句） | ✅ 已回填 |
| 本地压测 P99 首 token X s | M5.2 口径① | **✅ 已产出**（`reports/m5_loadtest_overhead.txt`，2026-08-04）：三档 619 稳态请求零错误，**平台自身开销 P99 ≤1.4s**（=TTFT P99 23s − 注入基线 21.6s；口径①=LatencyModelProvider 800ms+20tok/s 注入、TTFT 原始值失真声明在报）；真实上游对照口径② `m5_real_first_token.txt` P50 1306ms（800ms 模型偏乐观句已联动）。**已回填 05 §1**（限定语「本地压测（模拟上游延迟、60 并发 0 错误）平台自身开销」逐字入句） | ✅ 已回填 |
| 评测用例数 + 通过率基线 | M4.5 | **✅ 已产出**（`reports/eval_baseline_20260803.txt`）：**40 用例（三类 14/14/12）、稳定基线 38/40=95%**；口径=两条已知失败为强先验编造样本（okb-07 发票 90 天／iso-12 退款时效撞值）**如实保留不美化**——"95% 且能逐条说出那 5% 是什么"是比 100% 更硬的简历数字；judge 判分 spot-check ±1 一致率 100%（异族复评，`reports/m4_judge_spotcheck.txt`）。**已回填 05 §1**（「95% 且两条失败可逐条说明」句式） | ✅ 已回填 |
| 熔断恢复 X 秒闭合（04 M1 验收未尽项） | M5.4 | **✅ 已产出**（`reports/m5_breaker_recovery.txt`，2026-08-04）：三轮 **32.5/31.7/32.3s**（D8 定义；open TTL 30s 主导）。**已回填 05 §1**（「≤32.5s（三轮实测 31.7–32.5s）」） | ✅ 已回填 |
| 档内容灾实测（standard：qwen-plus→qwen-turbo；deepseek/glm 两版对象均随 #28 作废——M5.0 P6 修正） | M5.4 | **✅ 已产出**（`reports/m5_failover_qwen_tier.txt`，2026-08-04）：**20/20** 切换 qwen-turbo + ledger 20 行计费同步；C5/D9 预期行为段（熔断不开机理）在报。**已回填 05 §1**（「档内容灾切换 20/20」；05 的 DeepSeek 字样已按 #28 移除） | ✅ 已回填 |
