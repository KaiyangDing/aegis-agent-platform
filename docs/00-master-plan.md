...
| M4.4 | **离线质量评测**：eval_cases/eval_runs 表迁移（02 §3，种子集从文件形态迁入落表）；真实调用 + LLM-as-judge（strong 档），nightly/手动触发，单次预算上限写死在配置；三类用例（检索质量/端到端/对抗）各有判据文档 | L | 04 M4、02 §3 |
...
| M4.7 | **设计文档迁入 repo**：`docs/` 全量（含本文档）迁入仓库 `docs/`，此后文档变更走提交（"M5 前迁入"承诺在此兑现） | S | 记忆档案 |
