...
468. ⬜ notifier 的心跳探针为什么不能复用 is_closed()，也不能复用 poll_interval_s？
     （is_closed() 只认显式关闭——黑洞形态（对端消失无 FIN）下它永远 False，于是**失效
     模式（25s 级静默失聪）反而比设计好的降级模式（2s 轮询）慢一个量级且无日志**；
     SELECT 1 心跳把发现时延压到心跳周期量级、失败走既有重连/降级同一条路。不复用
     poll_interval_s：那个旋钮已身兼降级节拍（想调小）与重连退避（想调大）两职（(70)
     已知边界），再加"探测节拍"第三职=三个方向互相打架。一般化：**每个可独立调整的
     语义配一个旋钮，失效发现的速度要与降级路径的速度同数量级**）

469. ⬜ 断连时为什么必须唤醒**在途**等待者？（(67) 的"降级开关只对新来的人生效"）
     （降级判据是 wait_for 入口的 `_conn is None`——已经在 Event 上挂着的等待者判过了，
     连接死了没人 set 他们的 Event，只能熬满各自 timeout（首块 25s 量级）。修=降级/停机
     的 finally 遍历 _waiters 全部 set；被唤醒者按"通知不带数据、一律重查"的既有原则
     走一遍增量查询，随后再进 wait_for 时才看见降级态。伪唤醒安全让这个唤醒**不需要
     任何精确性**——这是 M3.10 选"通知只做提示"时就买下的红利，(67) 只是来兑现）

470. ⬜ TenantDirectory 缓存键为什么要加"环境身份"维度？副本化又防什么？
     （RLS 让"能读到什么"取决于环境身份；缓存命中=跳过回库=**跳过 RLS 复核**——键不含
     身份，身份 A 灌进的行会服务身份 B 的命中（当前请求路径不查 users 故不可达，一旦
     为"即时降权"接上 get_user 立刻是越权面）。键含身份后：每个身份只命中自己灌的行，
     miss 回库时 RLS 仍在场——**缓存变成"RLS 结论的备忘录"而非旁路**。副本化（deepcopy
     config）防的是另一件事：detached 实例被认证/预算/装配三消费方共享，谁就地改 config
     谁污染全进程 60s——两修合起来：缓存既不放大权限、也不传播突变）

471. ⬜ 为什么要专门写一个迁移去 REVOKE alembic_version？
     （M3.3 的 `GRANT ON ALL TABLES` 把迁移簿记表一并授给了低权应用角色，且该表无租户列
     不在 RLS 名单——aegis_app 可改迁移水位=让下次 upgrade 从错误位置重放，属基础设施
     完整性面。应用运行时没有任何路径需要碰它：回收是"粗粒度授权换维护便利"的代价
     结算。深挖点：DEFAULT PRIVILEGES 不回收——它只影响未来新表，而业务新表按 M3.3
     前置义务自带 RLS；证人测试跑在 rls_engine 世界（SELECT 也 42501），防"revoke 写了
     没生效"的假绿）

472. ⬜ 兜底建单失败的正确话术为什么是"零承诺"？（㊻ 的修复哲学）
     （兜底路径的依赖（mock 后端可用）比主路径**更多**，而"生病"与"下游不可用"高度
     相关——兜底恰恰在最可能失败的时刻运行。旧形态唯一话术自称"已生成工单"，把
     "失败也照发安抚"这条修法堵死（说了等于撒谎），于是失败=500 空响应=比不兜底
     信息量更少。修=新增 FALLBACK_LOOP_LIMIT_NO_TICKET：不提工单、给用户可自救的
     去向（稍后重试/人工热线）。一般化：**兜底话术是契约——先设计"兜底自身失败"
     那一档的措辞，才能让 catch 块有话可说**）

473. ⬜ migrate 为什么是 one-shot 服务而不是 api 启动时自跑迁移？
     （多副本世界 api 启动自跑=N 个副本抢同一份迁移（alembic 无内建互斥，撞了各种半态）；
     one-shot 服务+`service_completed_successfully` 依赖=**迁移恰跑一次、全部应用容器
     等它完成才起**——把"先迁库后起应用"从运维纪律变成编排结构。配套选型：应用容器
     restart=unless-stopped 与 pg/redis 同款（保住"手动 docker stop 看降级"的演示口径，
     M1.12/M2.12/M5.4 三处实录都依赖它）；migrate 是 "no"（one-shot 重启=重复跑空迁移，
     无害但噪音）。深挖：--scale api=3 的两道拦截（container_name 先撞、宿主端口后撞）
     =M5.3 上 Nginx 时要同时拆掉的两颗钉子）

474. ⬜ #48 的"至少一次投递"为什么要两个世界各测一次？102s 是怎么构成的？
     （补丁一开了 task_acks_late，但把 unacked 放回队列的是 kombu 的 restore 定时器
     （`loop.call_repeatedly(10, maybe_restore_messages)`），它挂在 event loop 上——
     celery `should_use_eventloop` 显式排除 Windows → 本地实录只能证"消息不丢"+手动
     restore（M4.0④b）；**"谁来定时调用"这一环只在 Linux/asynloop 世界存在**，容器化
     是它第一次可验证的时机（M4.7：kill -9 → 重启 → 零人工 102s 自动恢复至 DONE）。
     102s ≈ visibility_timeout 60s（消息从投递起算的可见性窗）+ 定时器 10s 节拍 +
     重启就绪与重新消费（3 批×3s 假服务延迟+调度余量）——数字能被机制逐项解释，
     才配写进凭证。连带红利：账本行数=理论批数（崩溃当批计量随事务回滚）=零重复计费）

475. ⬜ LoopBoundGuard 为什么分"严格抛/响亮警告"两档？替身为什么天然豁免？
     （跨 loop 单例陷阱三次现形三种修法（构造点显式传/工厂参数直通/安装缝），共性防线
     补的是"发现"不是"修复"。分档依据=各单例的**契约现状**：mock_client 的 docstring
     本就禁止跨 loop 复用→strict 抛人话；get_redis/shared_client 穿行于大量既有测试
     装置→一刀切抛会把"侥幸可用"（池子恰好在新 loop 里现建连接）的路径切死，先警告
     观察一个里程碑（989 全绿零误伤=分档实证）。替身豁免靠**对象身份重绑定**：防线盯
     "同一实例跨 loop"（keep-alive 连接绑旧 loop）——换了新实例（set_mock_client/
     monkeypatch）连接从零建，风险本体不存在，硬拦反而误伤每测新建的正确姿势）

## M5 · 压测与交付收口（2026-08-04 出题；scripts/loadtest/ + LatencyModelProvider + deploy/nginx + 四实验报告 + demo-script）

476. ⬜ 为什么 locust 的 SSE client 要自写？TTFT 是怎么采的？
     （locust 原生只有 HTTP 请求级计时，SSE 逐帧语义与"首 token 时刻"它一无所知；自写
     iter_sse_frames 纯函数（8 帧词汇、

 分帧、data 半行攒够再解析）+ AegisChatUser 在
     收到首个文本帧的墙钟点 fire 自定义 chat_first_token 事件——TTFT 是业务定义不是传输
     定义。另一半原因是 gevent×asyncio 不共戴天：locustfile 绝不 import aegis，客户端用
     纯同步 requests 流式读，隔离在进程边界（scripts/loadtest/ 全部 stdlib+requests））

477. ⬜ "平台自身开销"怎么从端到端延迟里剥出来？注入基线 21,600ms 是怎么构成的？
     （口径①：上游换成 LatencyModelProvider（首块 800ms+20tok/s+200tok 全部钉死）→ 端到端
     TTFT 减去"注入管线基线"剩下的就是平台开销（准入/锁/事件写/编排/SSE/nginx）。基线
     21.6s=classify 全流 10.8s + 主答首块 0.8s——classify 本应只取首词，但 OutputGuard 对
     无边界合成文本"测"×200 整段缓冲 flush，两段全流是实测形态而非理想形态；两条失真
     （合成文本无句界→守卫整段挂账；分诊回复与答案同长）在报告口径段如实声明，减法结果
     不受影响。教训：基线必须实测推导，拍脑袋的"理论管线"会把守卫行为漏在账外）

478. ⬜ 真实调用为什么不做高并发压测？
     （费用与厂商限流双约束（04 M5 原文理由原样声明）：高并发打真实上游=烧钱买一个受
     供应商配额而非本平台决定的数字；平台自身容量已由口径①（零成本、可复跑、控变量）
     覆盖，真实上游只需小样本分布（口径② N=100 串行）给 800ms 模型一个现实参照——实测
     P50 1306ms 说明模型偏乐观，这句限定语随口径①读数走。两口径正交：一个测平台、一个
     测世界，混在一起两个都不准）

479. ⬜ Nginx 反代 SSE 的两个坑是什么？与 L1 的块间空闲超时有什么同构关系？
     （坑一 proxy_buffering：默认攒满缓冲区才发，SSE 逐帧到达变成一坨延迟到达——off 才有
     流式；坑二 proxy_read_timeout：默认 60s 按"两次读之间的空闲"计时，长审批挂起/慢答
     案会被反代掐线——调大到 3600s（另 gzip off 防压缩缓冲）。同构：L1 对上游的两阶段超时
     （首块超时/块间空闲超时）与 nginx 对 upstream 的语义完全同族——流式链路上每一跳都要
     区分"还没开始吐"和"吐着吐着停了"，超时必须按空闲而非总时长计）

480. ⬜ 熔断恢复闭合时间由什么主导？实验怎么设计的？第一版实验踩了什么坑？
     （主导因子=open TTL 剩余（设计值 30s，TTL 即状态迁移）：闭合时间 ∈ (探针耗时,
     30s+探针]，随打开时刻的 TTL 相位浮动是预期——三轮 32.5/31.7/32.3s。设计：单候选路由
     +100% 注入攒账打开（注入在真实调用前拦截=零 token）→ 切无注入网关记 t_recover →
     0.5s 真实短探针至首成功且键清零。两版坑都在"簿记在流尾"：第一版首块即 aclose——
     on_success 不执行、fails=5 残留、半开探针令牌 120s 锁死；重跑版全量耗尽流才闭合。
     姊妹坑=(58) 家族第三例：裸网关驱动没配 tenant_context，计量被 RLS 静默拒收成空账，
     排练首遍暴露后两实验补配对重跑出真账）

481. ⬜ 同 provider 的档内 fallback 下熔断为什么全程不开？这暴露了什么？
     （熔断按 provider 记账（bailian 一本账）：qwen-plus 注入失败 +1 后，同 provider 的
     qwen-turbo fallback 成功即 on_success('bailian') 清零——账永远到不了阈值 5，20/20 全
     切换而 open 键从未出现（C5/D9 预期行为，demo 预答）。暴露 provider 粒度对"单模型
     坏死"的钝感：v1 显式接受（fallback 本身就是这层的防线），v2 细化到 provider:model
     ——故障注入靶已经是 provider:model 精确格式，细化有现成的键形）

482. ⬜ demo 的 replay 段到底在演示什么本质能力？
     （不是"能重放"这个动作，而是**同一条事件流的三重身份**：断点续跑的物质基础（恢复=
     重放到流尾接着写）、坐席排障的 trace（还原每步耗时/工具参数，展示层脱敏）、CI 的
     零 token 回放门（录制的事故现场每次提交都确定性重演一遍，行为等价性断言值守 prompt
     变更）。一份事实三个消费面，写入侧只付一次代价——这是"事件溯源在 Agent 场景不是
     过度设计"的最短证明，也是简历第一行放它的原因）
